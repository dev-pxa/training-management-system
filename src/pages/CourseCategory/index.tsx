...
interface CategoryTreeItem extends CourseCategoryRef {
  key: number;
...
type CategoryModalState =
  | { mode: 'create-primary' }
  | { mode: 'create-secondary'; parent: CourseCategoryRef }
  | { mode: 'edit'; category: CourseCategoryRef }
  | undefined;
...
const CourseCategoryPage: React.FC = () => {
  ...
  const loadCategories = useCallback(async () => {
    setLoading(true);
    try {
      const primaryResult = await getCourseCategories(0);
      if (primaryResult.code !== 0 || !Array.isArray(primaryResult.data)) {
        message.error(primaryResult.des || '获取课程分类失败');
        return;
      }
      const primaryCategories = primaryResult.data as CourseCategoryRef[];
      const childrenResults = await Promise.all(
        primaryCategories.map((category) => getCourseCategories(category.id)),
      );
      const categoryTree = primaryCategories.map((category, index) => {
        ...
        const children =
          childrenResult.code === 0 && Array.isArray(childrenResult.data)
            ? (childrenResult.data as CourseCategoryRef[]).map((item) => ({
                ...item,
                key: item.id,
              }))
            : [];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (modalState?.mode === 'edit') {
        result = await updateCourseCategory(modalState.category.id, {
          name: normalizedName,
        });
      } else if (modalState?.mode === 'create-secondary') {
        result = await addCourseCategory({
          parentId: modalState.parent.id,
          name: normalizedName,
        });
      } else {
        result = await addCourseCategory({ parentId: 0, name: normalizedName });
      }
      if (
        handleApiResponse(
          result,
  ...
  const handleDelete = async (category: CourseCategoryRef) => {
    try {
      const result = await deleteCourseCategory(category.id);
  ...
  const columns: any[] = [
    ...
    },
    {
      ...
      valueType: 'option',
      width: 300,
      render: (_: unknown, record: CourseCategoryRef) => (
        <Space size={0}>
          <Button
            type="link"
  ...
  return (
    <PageContainer header={{ title: '课程分类管理' }}>
